...
def get_coordinate(pixel_x, pixel_y):
    ...
    depth_f = int(depth_image[pixel_x, pixel_y])/1000 #this is in meters
...
def distance_depth(point_x, point_y):
    return int(depth_image[point_x, point_y])/1000
                ...
                for handLms in results.multi_hand_landmarks:
                    for id, lm in enumerate(handLms.landmark):
                        ...
                        if id == 9:
                            ...
                            cv2.putText(color_image, "x:{0} y:{1} z:{2} speed:{3}".format(rww_x, rww_y, rww_z, rww_z/seconds), (cx-100, cy-20), 0, 1, (255,182,193), 2)
