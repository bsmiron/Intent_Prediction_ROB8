function q = ikSolverUR5All_ads(pos,eul,qPrevious)
...
%DH parameters for the UR5 robot, these values are constant
...
for i = 1:size(solutions,1)
    ...
    if configurationDistance < bestConfigurationDistance
        ...
        for j = 1:6
            if (solution(j) > pi)
                solution(j) = solution(j) - 2* pi;
            elseif (solution(j) < -pi)
                solution(j) = solution(j) + 2* pi; 
            end
        end
    end
